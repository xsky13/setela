...
const { data: user } = await supabase.from("profiles").select('full_name').eq('id', props.entry.userId).single();
</script>

<template>
    <div v-if="props.entry.userId == props.user.id || props.user.admin" class="box !bg-white !flex justify-between items-start my-6">
        <div>
            ...
            <div class="text-sm text-gray-600">
                ...
                }} • Entrega de {{ user.full_name }}
            </div>
        </div>
        <div class="flex flex-col items-center">
            <div v-if="props.userIsProfesor" class="mb-2">
                <Modal :condition="true" text="Evaluar" title="Evaluar trabajo practico" ref="gradeHomeworkModalRef">
                    <form @submit.prevent="() =>
                            props.gradeHomework(
                                props.entry.id,
                                entryGrade,
                        ...
                        <div class="my-6">
                            ...
                            <input type="number" class="text-field" v-model="entryGrade" />
                        </div>
                        <button class="btn btn-primary w-full">
                            <img v-if="gradeHomeworkModalLoading" src="/loading_white.svg" width="20" />
            ...
            <div v-if="props.user?.id == entry.userId">
                <Modal :condition="props.user?.id == entry.userId"
                    ...
                    class="cursor-pointer" title="Eliminar entrega" ref="deleteEntryModalRef">
                    <p class="py-6">
                        Esta seguro que quiere eliminar esta entraga? Esta
                        acción es irreversible.
                    </p>
                    <button :disabled="modalLoading" class="btn btn-danger w-full" @click="() => props.deleteEntry(props.entryNumber, entry.id)
                        ">
                        <img v-if="modalLoading" src="/loading_white.svg" width="20" />
                        <span v-else>Eliminar</span>
                    </button>
                </Modal>
            </div>
            <div v-if="props.entry.grade"
